...
export const mapWalkin = (row) => {
  ...
  // Return Date mapping (for Rent-Out leads disguised as Walk-ins)
  const returnDate = (
    row["Return Date"] || row["return date"] || row["RETURN DATE"] ||
    row.returnDate || row.ReturnDate ||
    row["__EMPTY_8"] // Potential column position
  );

  // Booking No (for Booking/Rent-Out)
  const bookingNo = (
    row["Booking No"] || row["booking no"] || row["BOOKING NO"] ||
    row.bookingNo || row.BookingNo ||
    row["__EMPTY_0"] // Potential column position
  )?.trim();
  ...
  if (parsedReturnDate) {
    ...
    source = "Rent-out";
  } else if (bookingNo && status && status.toLowerCase().includes('book')) {
    // Only assume booking if status explicitly mentions it, otherwise might still be general
